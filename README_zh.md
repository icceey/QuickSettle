...
一个 [tModLoader](https://github.com/tModLoader/tModLoader) 的 Terraria Mod，在玩家于服务器聊天框中发送 `1` 时，会以“分帧时间切片”的后台方式结算世界中所有流动的液体，避免一次性长时间阻塞主线程，同时加速你的地形改造工程。

## 为什么做这个 Mod？

在 Terraria 中，挖掘方块后导致大面积的液体流动往往需要花费很长时间（特别是岩浆和蜂蜜，它们的流动十分缓慢），这不仅需要玩家耐心等待，还会导致服务器卡顿。尤其是在多人游戏和大型模组服务器中，这种情况更为明显。

你可以通过重启服务器或是开启命令行之后使用 `settle` 命令来结算液体，但我不想让朋友们反复退出-重进游戏（这体验并不好），所以我做了这个 mod。它会在多个游戏帧内分批调用 Terraria 的液体更新逻辑，让液体尽快完成结算，同时尽量避免一次性卡住整个主线程；你只需扣 1 就能做到，无需再退出游戏。
...
- 在游戏内聊天框中发送 `1`。该方式默认开启，如果你担心误触，可在服务端配置中关闭发送 `1` 的快捷触发。
- 输入 `/settle`。即使发送 `1` 的快捷触发被关闭，这个命令仍然可用。
